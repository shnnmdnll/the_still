...
// I-check kung may na-upload na ba ang guest ng valid ID bago ipakita ang confirmation
$idCheckStmt = $pdo->prepare("SELECT id_verification_status FROM users WHERE id = :id");
$idCheckStmt->execute([':id' => $_SESSION['user_id']]);
$idStatus = $idCheckStmt->fetchColumn();

if (empty($idStatus) || $idStatus === 'not_submitted') {
    header('Location: upload_id.php');
    exit();
}
...
if (!$unit_id) {
    styledError("Missing unit information. Please try booking again.");
}
...
// Check overlapping bookings (pending o confirmed lang ang naka-block)
$stmt = $pdo->prepare("SELECT * FROM bookings 
    WHERE unit_id = ? 
    AND status IN ('pending','confirmed')
    AND (check_in < ? AND check_out > ?)");
$stmt->execute([$unit_id, $check_out, $check_in]);

if ($stmt->rowCount() > 0) {
    styledError("Sorry, hindi available ang unit sa napiling dates.");
}

$stmt = $pdo->prepare("SELECT price FROM units WHERE id = ?");
$stmt->execute([$unit_id]);
$unit = $stmt->fetch();

if (!$unit) {
    styledError("Unit not found.");
}
