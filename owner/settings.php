...
$settings = $pdo->query("SELECT * FROM platform_settings LIMIT 1")->fetch(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="app-layout">
        <aside class="sidebar">
            ...
            <div class="sidebar-bottom">
                <a href="settings.php" class="nav-item muted active">
        ...
        <main class="main-content">
            <header class="topbar">
                <div>
                    <h1>Settings</h1>
                    <p>Platform configuration and account preferences</p>
                </div>
            ...
            <div class="dashboard-container">
                ...
                <div class="settings-panel">
                    ...
                    <form id="platformSettingsForm">
                        <div class="form-row">
                            ...
                            <input type="text" id="siteName" value="<?php echo htmlspecialchars($settings['site_name'] ?? ''); ?>" required>
                        </div>
                        <div class="form-row">
                            <label>Support Email</label>
                            <input type="email" id="supportEmail" value="<?php echo htmlspecialchars($settings['support_email'] ?? ''); ?>" required>
                        </div>
                        <div class="form-row">
                            <label>Commission Rate (%)</label>
                            <input type="number" id="commissionRate" step="0.01" min="0" max="100" value="<?php echo htmlspecialchars($settings['commission_rate'] ?? '10.00'); ?>" required>
                        </div>
                        <div class="form-row">
                            <label>Cancellation Policy</label>
                            <textarea id="cancellationPolicy" rows="3"><?php echo htmlspecialchars($settings['cancellation_policy'] ?? ''); ?></textarea>
                        </div>
                        <button type="submit" class="btn-save">Save Platform Settings</button>
                    </form>
                </div>
                ...
                <div class="settings-panel">
                    ...
                    <form id="accountForm">
                        <div class="form-row">
                            <label>Name</label>
                            <input type="text" id="accName" value="<?php echo htmlspecialchars($ownerName); ?>" required>
                        </div>
                        <div class="form-row">
                            <label>Email</label>
                            <input type="email" id="accEmail" value="<?php echo htmlspecialchars($ownerEmail); ?>" required>
                        </div>
                        <div class="form-row">
                            <label>New Password <span class="optional-label">(leave blank to keep current)</span></label>
                            <input type="password" id="accPassword" minlength="6" placeholder="••••••••">
                        </div>
    ...
    <script>
        document.getElementById('platformSettingsForm').addEventListener('submit', (e) => {
            e.preventDefault();
            fetch('../api/update_platform_settings.php', {
                method: 'POST',
            ...
            .then(res => res.json())
            .then(data => {
                alert(data.success ? data.message : (data.error || 'Something went wrong.'));
            })
        ...
        document.getElementById('accountForm').addEventListener('submit', (e) => {
            e.preventDefault();
            fetch('../api/update_owner_account.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'same-origin',
                body: JSON.stringify({
                    ...
                    new_password: document.getElementById('accPassword').value,
                })
            })
            .then(res => res.json())
            .then(data => {
