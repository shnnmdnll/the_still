...
// ===== 1. HOST APPLICATIONS (pending) =====
if (empty($typeFilter) || $typeFilter === 'application') {
    $stmt = $pdo->query("
        SELECT id, applicant_name, business_name, submitted_at
    ...
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
        $notifications[] = [
            ...
            'message' => htmlspecialchars($row['applicant_name']) . ' applied to become a host' . (!empty($row['business_name']) ? ' — "' . htmlspecialchars($row['business_name']) . '"' : ''),
            'timestamp' => $row['submitted_at'],
            'link' => 'host_applications.php',
        ];
    }
}

// ===== 2. MAINTENANCE REPORTS (pending) =====
if (empty($typeFilter) || $typeFilter === 'maintenance') {
    $stmt = $pdo->query("
        SELECT mr.id, mr.description, mr.reported_at, un.name AS unit_name
    ...
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
        $notifications[] = [
            ...
            'message' => htmlspecialchars($row['unit_name']) . ': ' . htmlspecialchars(mb_strimwidth($row['description'] ?? '', 0, 80, '...')),
...
if (empty($typeFilter) || $typeFilter === 'booking') {
    $stmt = $pdo->query("
        SELECT b.id, b.status, b.check_in, un.name AS unit_name, g.name AS guest_name, b.check_in AS ts
        FROM bookings b
        JOIN units un ON un.id = b.unit_id
        JOIN users g ON g.id = b.user_id
        WHERE b.check_in >= CURRENT_DATE - INTERVAL '7 days'
        ORDER BY b.id DESC
        LIMIT 20
    ");
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
        $notifications[] = [
            'type' => 'booking',
            'title' => 'New Booking (' . ucfirst($row['status']) . ')',
            'message' => htmlspecialchars($row['guest_name']) . ' booked ' . htmlspecialchars($row['unit_name']) . ' for ' . htmlspecialchars($row['check_in']),
            'timestamp' => $row['ts'],
            'link' => 'bookings.php',
        ];
    }
...
// I-sort lahat by timestamp, newest first
...
function typeIcon($type) {
    switch ($type) {
        case 'application':  return ['📋', '#c98a1f'];
...
<head>
    <meta charset="UTF-8">
    <title>Notifications & Alerts — Owner Dashboard</title>
        ...
        <main class="main-content">
            ...
            <div class="dashboard-container">
                ...
                <div class="notif-tabs">
                    ...
                    <a href="?type=application" class="notif-tab <?php echo $typeFilter === 'application' ? 'active' : ''; ?>">Host Applications</a>
                    ...
                    <div class="notif-feed">
                        ...
                        ?>
                            <a href="<?php echo $n['link']; ?>" class="notif-card">
                                ...
                                <div class="notif-time"><?php echo timeAgo($n['timestamp']); ?></div>
                            </a>
                        <?php endforeach; ?>
                    </div>
                <?php endif; ?>
